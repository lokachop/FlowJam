...
FLK3D.FPSPath = FLK3D.GetRootDirectory() .. "external.fps."
...
local function addBodyToWorld(body)
	local lk3d_univ = FLK3D.GetCurrUniverse()
	local pUniv = getPhysicsWorld(lk3d_univ)
	if not pUniv then
		pUniv = initPhysicsWorld(lk3d_univ)
	end
...
function FLK3D.GetPhysicsBodyFromName(name)
	local lk3d_univ = FLK3D.GetCurrUniverse()
	if not lk3d_univ["physics_objects"] then
		return
	end

	return lk3d_univ["physics_objects"][name]
end

function FLK3D.GetPhysicsColliderFromName(name)
	local lk3d_univ = FLK3D.GetCurrUniverse()
	if not lk3d_univ["physics_colliders"] then
		return
	end

	return lk3d_univ["physics_colliders"][name]
end
...
function FLK3D.AddPhysicsBodyToUniv(name)
	local lk3d_univ = FLK3D.GetCurrUniverse()
	if not getPhysicsWorld(lk3d_univ) then
		pUniv = initPhysicsWorld(lk3d_univ)
	end
	...
	lk3d_univ["physics_objects"][name] = body
	lk3d_univ["physics_colliders"][name] = coll
...
function FLK3D.DebugRenderPhysicsObjects()
	...
	for k, v in pairs(lk3d_univ["physics_objects"]) do
		if FLK3D.GetPhysicsBodyStatic(k) then
			goto _cont
		end


		if not lk3d_univ["objects"][k] then
			...
			FLK3D.SetObjectFlag(k, "SHADING_SMOOTH", true)
	...
	for k, v in pairs(debugObjects) do
		if lk3d_univ["physics_objects"][k] == nil then
...
function FLK3D.PhysicsThink(dt)
	...
	local pWorld = getPhysicsWorld(lk3d_univ)
	if not pWorld then
		return
	end
