...
local box_render = PIXELBOX.new(term.current())
function FLK3D.RenderRTToScreen()
	...
	for i = 0, (rtW * rtH) - 1 do
		...
		box_render:set_pixel(xc, yc + 1, cont)
	end
	...
	if FLK3D.Debug then
		local _dbgRes, _dbgFPS, _dbgFrameTime, _dbgOBJCount, _dbgVertCount, _dbgTriCount, _dbgFragments = FLK3D.GetRasterDebugInfo()
		...
		term.write("AFPS: " .. string.format("%.2f", _dbgFPS))

		term.setCursorPos(1, 4)
		term.write("OBJ : " .. _dbgOBJCount)

		term.setCursorPos(1, 5)
		term.write("TRIS: " .. _dbgTriCount)

		term.setCursorPos(1, 6)
		term.write("VERT: " .. _dbgVertCount)

		term.setCursorPos(1, 6)
		term.write("FRAG: " .. _dbgFragments)


		FLK3D.ResetRasterDebug()
	end
end