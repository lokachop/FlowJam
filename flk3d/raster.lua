...
local _div = 32
local bayer4 = {
	 0 / _div,  8 / _div,  1 / _div,  9 / _div,
	12 / _div,  4 / _div, 13 / _div,  5 / _div,
	 3 / _div, 11 / _div,  2 / _div, 10 / _div,
	15 / _div,  7 / _div, 14 / _div,  6 / _div,
}
...
local _TEX_BAYER = 1
local _TEX_RANDOM = 2
local _TEX_LINEAR = 9999

local _table = {255, 0, 0}
local _white = colors.white
local _wfCol = colors.green --{0, 255, 0}
...
local _px1, _py1 = 0, 0
local _px2, _py2 = 0, 0
function FLK3D.Raster_SetPositions(x0, y0, x1, y1, x2, y2)
	_px0 = x0
	_py0 = y0
	_px1 = x1
	_py1 = y1
...
local _colHigh = 1
local _colShade = 1
local _colThreshold = 0.5
function FLK3D.Raster_SetColourParams(colHigh, colShade, colThreshold)
	_colHigh = colHigh
	_colShade = colShade
...
local _sv1 = 0
local _sv2 = 0
function FLK3D.Raster_SetShadeValue(sv0, sv1, sv2)
	_sv0 = sv0
	_sv1 = sv1
	_sv2 = sv2
end
...
local _vw1 = 0
local _vw2 = 0
function FLK3D.Raster_SetW(vw0, vw1, vw2)
	_vw0 = vw0
	_vw1 = vw1
	_vw2 = vw2
end

local _textured = false
function FLK3D.Raster_SetTextured(bool)
	_textured = bool
end

local _uvu0, _uvv0 = 0, 0
local _uvu1, _uvv1 = 0, 0
local _uvu2, _uvv2 = 0, 0
function FLK3D.Raster_SetUVs(u0, v0, u1, v1, u2, v2)
	_uvu0, _uvv0 = u0, v0
	_uvu1, _uvv1 = u1, v1
	_uvu2, _uvv2 = u2, v2
end

local _texdata = {data = {2, 2},
[0] = 1,
[1] = 2,
[2] = 2,
[3] = 1,
...
function FLK3D.Raster_SetTextureData(tdata)
	_texdata = tdata
end

local _lightmapped = false
function FLK3D.Raster_SetLightmapped(bool)
	_lightmapped = bool
end

local _lmu0, _lmv0 = 0, 0
local _lmu1, _lmv1 = 0, 0
local _lmu2, _lmv2 = 0, 0
function FLK3D.Raster_SetUVsLM(u0, v0, u1, v1, u2, v2)
	_lmu0, _lmv0 = u0, v0
	_lmu1, _lmv1 = u1, v1
	_lmu2, _lmv2 = u2, v2
end
...
function FLK3D.RenderTriangleParams()
	local x0, y0  = _px0, _py0
	local x1, y1  = _px1, _py1
	local x2, y2  = _px2, _py2
	...
	local minX = math_min(x0, x1, x2)
	local minY = math_min(y0, y1, y2)
	local maxX = math_max(x0, x1, x2)
	local maxY = math_max(y0, y1, y2)

	minX = math_max(minX, 0)
	minY = math_max(minY, 0)
	maxX = math_min(maxX, rtW - 1)
	maxY = math_min(maxY, rtH - 1)

	local texW, texH = 0, 0

	if _textured then
		texW, texH = _texdata.data[1], _texdata.data[2]
	end
	...
	for y = minY, maxY do
		for x = minX, maxX do
			...
			if w0 ~= w0 or w1 ~= w1 or w2 ~= w2 then
				goto _contBary
			end

			local wCalc = -((w0 * _vw0) + (w1 * _vw1) + (w2 * _vw2))
			local dCalc = (1 / wCalc)
			...
			if _textured then
				local uCalc = ((w0 * _uvu0) + (w1 * _uvu1) + (w2 * _uvu2))
				local vCalc = ((w0 * _uvv0) + (w1 * _uvv1) + (w2 * _uvv2))
	...
	for i = 1, charCount do
		...
		for x = 1, 6 do
			...
			for y = 0, 6 do
				...
				local shouldPlace = bit32.band(tblDataRow, bit32.lshift(1, y))
